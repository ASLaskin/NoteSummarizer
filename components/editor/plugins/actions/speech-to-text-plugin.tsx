...
function SpeechToTextPluginImpl() {
  ...
  // Check for speech recognition support on the client side only
  useEffect(() => {
    if (CAN_USE_DOM) {
      setIsSupported(
  ...
  useEffect(() => {
    if (isEnabled && recognition.current === null && isSupported) {
  ...
  }, [SpeechRecognition, editor, isEnabled, report, isSupported])
  ...
  // Don't render anything until we know if speech recognition is supported
  if (!isSupported) {
    return null
  }
